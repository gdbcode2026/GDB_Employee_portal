...
/**
 * Publishes {@code document.uploaded.v1} and consumes {@code EMPLOYEE_DEACTIVATED} per the
 * documented consumer table (docs/architecture/COMMUNICATION.md) - no other event type is
 * bound here. Reuses the exact exchange/DLX names already established by audit-service's topology.
 */
@Configuration
public class DocumentMessagingTopology {

    private static final String EXCHANGE = "gdb.domain.events";
    private static final String DLX = "gdb.domain.dlx";
    private static final String QUEUE = "document.employee-events.v1";
    private static final String DLQ = "document.employee-events.v1.dlq";
