...
import com.growdigitalbridge.attendance.domain.OutboxEvent;
import com.growdigitalbridge.attendance.repository.OutboxEventRepository;
...
class OutboxRelayTest {
    ...
    void publishesUnpublishedEventsAndMarksThemPublished() throws Exception {
        OutboxEvent event = event("attendance.finalized.v1");
        when(repository.findByPublishedAtIsNullOrderByOccurredAtAsc(any())).thenReturn(List.of(event));

        relay().relay();

        verify(rabbitTemplate).convertAndSend(eq("gdb.domain.events"), eq("attendance.finalized.v1"), any(DomainEvent.class));
    ...
    void oneFailingEventDoesNotBlockOthersInTheSameBatch() throws Exception {
        OutboxEvent failing = event("attendance.finalized.v1");
        OutboxEvent succeeding = event("attendance.regularization-approved.v1");
        when(repository.findByPublishedAtIsNullOrderByOccurredAtAsc(any())).thenReturn(List.of(failing, succeeding));
        doThrow(new AmqpException("broker unavailable") { })
                .when(rabbitTemplate).convertAndSend(eq("gdb.domain.events"), eq("attendance.finalized.v1"), any(Object.class));
