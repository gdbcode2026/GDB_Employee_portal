...
import com.growdigitalbridge.leave.repository.OutboxEventRepository;
...
class OutboxRelayTest {
    ...
    void publishesUnpublishedEventsAndMarksThemPublished() throws Exception {
        OutboxEvent event = event("leave.requested.v1");
        when(repository.findByPublishedAtIsNullOrderByOccurredAtAsc(any())).thenReturn(List.of(event));

        relay().relay();

        verify(rabbitTemplate).convertAndSend(eq("gdb.domain.events"), eq("leave.requested.v1"), any(DomainEvent.class));
    ...
    void oneFailingEventDoesNotBlockOthersInTheSameBatch() throws Exception {
        OutboxEvent failing = event("leave.requested.v1");
        OutboxEvent succeeding = event("leave.approved.v1");
        when(repository.findByPublishedAtIsNullOrderByOccurredAtAsc(any())).thenReturn(List.of(failing, succeeding));
        doThrow(new AmqpException("broker unavailable") { })
                .when(rabbitTemplate).convertAndSend(eq("gdb.domain.events"), eq("leave.requested.v1"), any(Object.class));
