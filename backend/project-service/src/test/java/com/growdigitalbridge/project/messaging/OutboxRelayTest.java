...
class OutboxRelayTest {
    ...
    private OutboxEvent event(String eventType) throws Exception {
        String payload = objectMapper.writeValueAsString(Map.of("projectId", UUID.randomUUID().toString()));
    ...
    void publishesUnpublishedEventsAndMarksThemPublished() throws Exception {
        OutboxEvent event = event("project.created.v1");
        when(repository.findByPublishedAtIsNullOrderByOccurredAtAsc(any())).thenReturn(List.of(event));

        relay().relay();

        verify(rabbitTemplate).convertAndSend(eq("gdb.domain.events"), eq("project.created.v1"), any(DomainEvent.class));
    ...
    void oneFailingEventDoesNotBlockOthersInTheSameBatch() throws Exception {
        OutboxEvent failing = event("project.created.v1");
        OutboxEvent succeeding = event("task.created.v1");
        when(repository.findByPublishedAtIsNullOrderByOccurredAtAsc(any())).thenReturn(List.of(failing, succeeding));
        doThrow(new AmqpException("broker unavailable") { })
                .when(rabbitTemplate).convertAndSend(eq("gdb.domain.events"), eq("project.created.v1"), any(Object.class));
